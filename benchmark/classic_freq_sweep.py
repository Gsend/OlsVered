"""
benchmark/classic_freq_sweep.py

Classic K-FAC factor_update_freq sweep at champion cell
(lr=2e-3, damping=1e-4, mom=0.7, gamma=0.9).  Same setup as
vered_freq_sweep.py but ClassicKFAC variant -- faster per cell, so a
cheap baseline before the Vered version finishes.

freq=20 omitted -- matched-screen Classic at freq=20 is 922 ppl.

4 cells x ~10 min = ~40 min.

Output: benchmark/results/classic_freq{freq}_d1e-04_lr2e-03_s1000_const.json
"""
...
def out_path(freq):
    return OUT / f"classic_freq{freq}_d1e-04_lr2e-03_s1000_const.json"
...
def main():
    ...
    print(f"Classic freq sweep at lr={FIXED['kfac_lr']}, damping={FIXED['damping']}")
    tlf, vl, vocab = build_data(device)
    pad = vocab - 1

    runs = [run_one(f, tlf, vl, vocab, pad, device, hw) for f in FREQS]

    print("\n=== summary (Classic, lr=2e-3, damping=1e-4) ===")
    print(f"  {'freq':>6}  {'final_ppl':>10}  {'slope/100':>10}")
    print(f"  {'20':>6}  {'922':>10}  {'(matched-screen baseline)':>25}")
    for f in FREQS:
