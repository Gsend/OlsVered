"""
diagnostic/runners/arch_coverage/e1_deep_mlp.py
================================================

E1: DeepMLP-10L on MNIST classification.
...
Standardized battery:
  ...
  B-subset         — layer-subset sweep {fc10 only, fc10+fc9, ..., all 10}
  B-K-vs-N         — {naive, kfac_a} × {no_mom, mom_match} on random init

Hyperparameters (from arch_coverage plan):
  n_samples  = 16 384
  seed       = 42
  gt_margin  = 5.0
  ols_lambda = 1e-4
  ols_eps    = 1e-4
  iterations = 10 (B-iter)

Outputs: benchmark/results/diagnostic/arch_coverage/e1/{battery_item}.json
Weights: benchmark/weights/arch_coverage/deep_mlp_10l.pt

Usage:
    python -m diagnostic.runners.arch_coverage.e1_deep_mlp
    python -m diagnostic.runners.arch_coverage.e1_deep_mlp --skip-train
"""
...
RESULTS_DIR = (
    REPO_ROOT / "benchmark" / "results" / "diagnostic" / "arch_coverage" / "e1"
)
WEIGHTS_DIR = REPO_ROOT / "benchmark" / "weights" / "arch_coverage"
WEIGHTS_PATH = WEIGHTS_DIR / "deep_mlp_10l.pt"
...
def _build_loader_list(dataset, max_samples: int, batch_size: int = 128):
    """Return (images_list, labels_tensor) from the first max_samples of dataset."""
    ...
    return images, labels  # images: list[Tensor], labels: 1-D Tensor


def _layer_name(chain_idx: int) -> str:
    """Map chain_idx (0=deepest=fc10) → fc name string."""
...
def b_train(train_set, test_loader, *, device: torch.device) -> dict:
    print("\n[E1] === B-train: training DeepMLP-10L ===")
    model = DeepMLP(n_layers=N_LAYERS, hidden_dim=HIDDEN_DIM).to(device)
    ...
    print(f"  [B-train] test_acc={acc:.4f}  wall={wall_s:.1f}s  "
          f"weights -> {WEIGHTS_PATH}")
    result = {
        "battery": "B-train", "exp_id": "e1",
...
def b_distill(trained_model: DeepMLP, loader_list: list,
              test_loader, *, device: torch.device) -> dict:
    """Single-pass TP+OLS starting from trained weights; deepest target = model's own output."""
    print("\n[E1] === B-distill: self-distillation from trained model ===")
    ...
    result = {
        "battery": "B-distill", "exp_id": "e1",
        "test_acc": acc, "wall_s": wall_s,
        "per_layer_residuals": per_layer_res,
        "per_layer_info": {str(k): v for k, v in res.per_layer_info.items()},
    }
...
def b_retrain_rand(train_set, test_loader, *, device: torch.device,
                   n_samples: int) -> dict:
    """Single-pass TP+OLS from random init, GT one-hot logit targets."""
    print("\n[E1] === B-retrain-rand: random init + GT labels, single pass ===")
    ...
    print(f"  [B-retrain-rand] test_acc={acc:.4f}  "
          f"delta={acc - rand_acc:+.4f}  wall={wall_s:.1f}s")
    for name in [f"fc{i + 1}" for i in range(N_LAYERS)]:
        r = per_layer_res.get(name)
        print(f"    {name}: residual={r:.3g}" if r is not None else f"    {name}: --")
    result = {
        "battery": "B-retrain-rand", "exp_id": "e1", "seed": SEED,
        "random_init_acc": rand_acc, "retrained_acc": acc,
        "delta": acc - rand_acc, "wall_s": wall_s,
        "per_layer_residuals": per_layer_res,
        "per_layer_info": {str(k): v for k, v in info.items()},
    }
...
def b_residual_profile(distill_result: dict, retrain_rand_result: dict) -> dict:
    """Combine distill and retrain-rand per-layer residuals into a profile."""
    print("\n[E1] === B-residual-profile ===")
    ...
    result = {
        "battery": "B-residual-profile", "exp_id": "e1",
...
def _plot_residual_profile(profile: dict, layer_names: List[str]) -> None:
    ...
    ax.set_ylabel("OLS target residual ||pred-t||/||t||")
    ax.set_title("E1 DeepMLP-10L: per-layer residual profile")
    ax.legend()
    ax.set_yscale("log")
    fig.tight_layout()
...
def b_iter(train_set, test_loader, *, device: torch.device, n_samples: int) -> dict:
    """10 iterations of full-chain TP+OLS from random init. Tracks acc + residuals per iter."""
    print(f"\n[E1] === B-iter: {N_ITER} iterations from random init ===")
    torch.manual_seed(SEED)
    model = DeepMLP(n_layers=N_LAYERS, hidden_dim=HIDDEN_DIM).to(device)
    rand_acc = _eval(model, test_loader, device)
    layers_df = model.get_layers_deepest_first()
    ...
    for it in range(N_ITER):
        ...
        iter_residuals.append(
            {_layer_name(k): v["target_residual"] for k, v in info.items()}
        )
    ...
    result = {
        "battery": "B-iter", "exp_id": "e1",
        "seed": SEED, "n_iterations": N_ITER,
...
def _plot_iter_trajectory(iter_accs: List[float]) -> None:
    ...
    ax.set_title("E1 DeepMLP-10L: B-iter accuracy trajectory")
    fig.tight_layout()
...
def _run_subset_retrain(
    ...
    *,
    ...
    """Retrain a named subset of layers from trained_model with GT targets. Returns test acc."""
    work_model = copy.deepcopy(trained_model)
    # deepest-first names and corresponding work_model layers
    ...
    # Back-prop chain through all layers using starting weights
    ...
    for step_idx, layer in enumerate(df_layers):
        ...
        b_old = (
            layer.bias.detach().to(device=a_in_t.device, dtype=a_in_t.dtype)
    ...
    # OLS sweep shallowest → deepest, re-capturing X from partially-updated model
    target_set = set(target_layer_names)
    for sweep_pos in range(N_LAYERS):
        chain_idx = N_LAYERS - 1 - sweep_pos      # shallowest first
        ...
        with torch.no_grad():
            layer.weight.copy_(
                W_new.to(device=layer.weight.device, dtype=layer.weight.dtype)
            )
            if layer.bias is not None and b_new is not None:
                layer.bias.copy_(
                    b_new.to(device=layer.bias.device, dtype=layer.bias.dtype)
                )
...
def b_subset(
    ...
    *,
    ...
    """Layer-subset sweep: {fc10 only} → {fc10+fc9} → ... → {all 10 layers}."""
    print("\n[E1] === B-subset: layer-subset sweep from trained model ===")
    ...
    for k in range(1, N_LAYERS + 1):
        ...
        print(f"  {label} ({k:2d} layers): acc={acc:.4f}  "
              f"delta={acc - trained_acc:+.4f}  ({wall_s:.1f}s)")

    result = {
        "battery": "B-subset", "exp_id": "e1",
...
def _plot_subset_curve(subset_results: dict, trained_acc: float) -> None:
    ...
    ax.axhline(trained_acc, color="gray", linestyle="--", label=f"trained baseline ({trained_acc:.3f})")
    ax.set_xlabel("# layers retrained (deepest-first)")
    ax.set_ylabel("test accuracy")
    ax.set_title("E1 DeepMLP-10L: B-subset layer-sweep")
    ax.legend()
    fig.tight_layout()
...
def b_k_vs_n(train_set, test_loader, *, device: torch.device, n_samples: int) -> dict:
    """4-way comparison: {naive, kfac_a} × {no_mom, mom_match} from random init."""
    print("\n[E1] === B-K-vs-N: method × momentum comparison ===")
    torch.manual_seed(SEED)
    base_model = DeepMLP(n_layers=N_LAYERS, hidden_dim=HIDDEN_DIM).to(device)
    ...
    for method, mean_corr, cov_corr, label in combos:
        ...
        combo_results[label] = {
            "method": method,
            "correct_target_mean": mean_corr,
            "correct_target_cov": cov_corr,
            "acc": acc,
            "delta": acc - rand_acc,
            "wall_s": wall_s,
        }
    ...
    result = {
        "battery": "B-K-vs-N", "exp_id": "e1",
        "seed": SEED, "random_init_acc": rand_acc,
...
def main(argv=None):
    ...
    print(f"[E1] DeepMLP-{N_LAYERS}L  device={device}  "
          f"n_samples={args.n_samples}  hidden_dim={HIDDEN_DIM}")
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)

    print("[E1] loading MNIST...")
    ...
    if args.skip_train and WEIGHTS_PATH.exists():
        print(f"[E1] loading cached weights from {WEIGHTS_PATH}")
        trained_model = DeepMLP(n_layers=N_LAYERS, hidden_dim=HIDDEN_DIM).to(device)
        trained_model.load_state_dict(
            torch.load(WEIGHTS_PATH, map_location=device, weights_only=True)
        )
        trained_acc = _eval(trained_model, test_loader, device)
        print(f"[E1] loaded; test_acc={trained_acc:.4f}")
        train_result = {
            "battery": "B-train", "test_acc": trained_acc,
    ...
    # Shared loader + GT targets for battery items that use them
    ...
    if not args.skip_distill:
        distill_result = b_distill(
            trained_model, loader_list, test_loader, device=device
        )
    ...
    if not args.skip_subset:
        b_subset(
            trained_model, gt_targets, loader_list, test_loader,
    ...
    t_acc = train_result.get("test_acc", "?")
    d_acc = distill_result.get("test_acc", "?")
    rr_acc = retrain_rand_result.get("retrained_acc", "?")
    print(f"\n[E1] === COMPLETE ===")
    print(f"  B-train acc:       {t_acc}")
    print(f"  B-distill acc:     {d_acc}")
    print(f"  B-retrain-rand acc:{rr_acc}")
